...
class RecetasFragment : Fragment() {
    ...
    private fun actualizarUI(state: RecetasUiState) {
        ...
        if (binding.editBuscarReceta.text.toString() != state.currentQuery) {
            binding.editBuscarReceta.setText(state.currentQuery)
        }
        
        for (i in 0 until binding.chipGroupCategorias.childCount) {
            val chip = binding.chipGroupCategorias.getChildAt(i) as Chip
            if (chip.text.toString() == state.currentCategory) {
                chip.isChecked = true
            }
        }

        // Manejar visibilidad del detalle para que sobreviva a la rotación
        if (state.selectedReceta != null) {
            mostrarDetalleRecetaUI(state.selectedReceta, state.isLoading)
        } else {
    ...
    private fun mostrarDetalleRecetaUI(receta: Receta, isLoading: Boolean) {
        binding.layoutListaRecetas.visibility = View.GONE
        binding.detalleRecetaContainer.visibility = View.VISIBLE
        

        if (binding.detalleRecetaContainer.childCount == 0) {
